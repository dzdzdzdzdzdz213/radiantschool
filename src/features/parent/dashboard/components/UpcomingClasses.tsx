      ...
      {data.length === 0 ? (
        ...
        <div className="space-y-2">
          {data.slice(0, 8).map((item, idx) => (
            <div key={`${item.id}-${item.childId}-${idx}`} className="flex items-center gap-3 rounded-xl bg-accent/50 p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground truncate">{item.childName} · {item.roomName}</p>
              </div>
            </div>
          ))}
        </div>
