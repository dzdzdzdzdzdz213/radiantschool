...
export default function AiAssistant() {
  ...
  const suggestions = AI_SUGGESTIONS[role] ?? [];
  const title = AI_ROLE_LABEL[role] ?? 'Radiant AI';
        ...
        <div className="fixed bottom-24 right-6 z-50 flex h-[560px] w-[calc(100vw-3rem)] max-w-sm flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl animate-in slide-in-from-bottom-8 duration-300">
          <div className="flex items-center gap-3 border-b border-border bg-gradient-to-r from-indigo-500 via-purple-500 to-fuchsia-500 px-4 py-3 text-white">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-white/80 leading-tight">Piloté par Gemini · données temps réel</p>
            </div>
            <button
              onClick={reset}
            ...
            {messages.length === 0 ? (
              <div className="flex h-full flex-col justify-center gap-4">
                <div className="space-y-1 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">Posez une question ou choisissez une suggestion.</p>
                </div>
          ...
          <div className="border-t border-border p-3">
            ...
            <p className="mt-1.5 flex items-center gap-1 text-[10px] text-muted-foreground">
              <AlertCircle className="h-3 w-3" /> Les réponses sont basées sur les données réelles de l'école.
            </p>
          </div>
        </div>
      )}
