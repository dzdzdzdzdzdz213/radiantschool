-- ============================================================
-- ERP Tutoring Center — Schema Migration 001 (reordered)
-- ============================================================
...
-- Levels & Subjects (must be before students)
CREATE TABLE levels (
...
SELECT
    COALESCE((SELECT SUM(amount) FROM payments WHERE payment_date >= DATE_TRUNC('month', NOW())), 0) AS total_revenue,
    COALESCE((SELECT COUNT(*) FROM students WHERE id IN (SELECT DISTINCT student_id FROM course_enrollments WHERE status = 'active')), 0) AS active_students,
    COALESCE((SELECT COUNT(*) FROM users WHERE role = 'student' AND created_at >= DATE_TRUNC('month', NOW()) AND deleted_at IS NULL), 0) AS new_students_month,
    COALESCE((SELECT ROUND(AVG(current_enrollments::DECIMAL / NULLIF(capacity, 0)) * 100) FROM courses WHERE status = 'active'), 0) AS occupancy_rate,
    COALESCE((SELECT COUNT(*) FROM users WHERE status = 'pending'), 0) AS pending_approvals,
    COALESCE((SELECT COUNT(*) FROM invoices WHERE status IN ('unpaid', 'partially_paid')), 0) AS unpaid_invoices;
...
('center.name', '"Radiant Learning"', 'Nom du centre'),
