...
interface AuthContextType {
  ...
  signUp: (email: string, password: string, firstName: string, lastName: string, role: string, options?: SignUpOptions) => Promise<{ error?: string }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (email: string, password: string, firstName: string, lastName: string, role: string, options?: SignUpOptions) => {
    ...
    if (data.user) {
      ...
      if (role === 'student') {
        const regNumber = `STU-${String(Date.now()).slice(-8)}`;
